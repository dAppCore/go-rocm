...
When the caller does not pass `WithContextLen`, loaders preserve the model's advertised context window. Use an explicit `WithContextLen` or a benchmark policy context when a smoke test must constrain KV-cache memory on a smaller card.
