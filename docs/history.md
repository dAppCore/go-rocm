...
- Wire service mounts now include chat completions, non-streaming Responses, Anthropic Messages, Ollama chat/generate, capability, cache, cancel, embeddings, and rerank endpoints without provider policy, with ROCm mux coverage for cache warm/stats/clear, cache-warm request validation, and scheduled-model cancel routing.
